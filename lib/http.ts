...
type WocResponse<T> = T extends Record<string, unknown>
  ? {
      [P in keyof T]: T[P] extends (infer U)[]
        ? WocResponse<U>[]
        : T[P] extends Readonly<infer U>[]
        ? Readonly<WocResponse<U>>[]
        : P extends 'createdAt' | 'updatedAt'
        ? Date
        : WocResponse<T[P]>
    }
  : T
...
type WocQuery<T> = {
  [P in keyof T]: T[P] extends [] | Record<string, unknown> ? string : WocQuery<T[P]>
}
