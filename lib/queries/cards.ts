...
export const fromGetCardKey = (key: QueryKey) =>
  key[0] === "getCard" ? (key[1] as GetCardQuery) : null;

export const listCardsKey = (query: ListCardsQuery) => ["listCards", query];
export const fromListCardsKey = (key: QueryKey) =>
  key[0] === "listCards" ? (key[1] as ListCardsQuery) : null;
    ...
    },
    {
      onSuccess: (updates, variables) => {
        ...
        updateQueriesData<ListCardsData>(
          ...
          (listCardsData) =>
            mergeById(listCardsData, { ...updates, id: variables.cardId })
        );
      },
    }
  );
            ...
            predicate: keyPredicate(
              ...
              (query) =>
                (query.owners === undefined
                  ? true
                  : query.owners.includes(card.ownerId)) &&
                (query.parents === undefined
                  ? true
                  : card.parentId !== null &&
                    query.parents.includes(card.parentId)) &&
    ...
    },
    {
      onSuccess: (_void, variables) => {
        ...
        updateQueriesData<GetCardData>(
          ...
          (getCardData) => ({
            ...getCardData,
            childrenOrder: filterSync(
              getCardData.childrenOrder,
              (id) => id !== variables.cardId
            ),
          })
        );
