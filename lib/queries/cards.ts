...
export function useCard(query: GetCardQuery, options?: { initialData?: GetCardData }) {
  return useQuery(getCardKey(query), async () => callGetCard(query), {
    cacheTime: Infinity,
    staleTime: Infinity,
...
export function useCards(
  ...
  return useQuery(listCardsKey(query), async () => callListCards(query), {
    cacheTime: Infinity,
    staleTime: Infinity,
    initialData: options?.initialData,
...
export function useUpdateCard() {
  ...
  return useMutation(
    async (data: UpdateCardBody) => {
      return callUpdateCard(data)
    },
    {
      onSuccess: (updates, variables) => {
        // Update the GetCard query for the card itself
        updateQueryData<GetCardData>(queryClient, getCardKey({ cardId: variables.cardId }), (getCardData) => ({
          ...getCardData,
          ...updates,
        }))
...
export function useCreateCard() {
  const queryClient = useQueryClient()
  return useMutation(
    async (data: CreateCardBody) => {
      return callCreateCard(data)
    },
    {
      onSuccess: (card, variables) => {
            ...
            predicate: keyPredicate(
              ...
              (query) =>
                ...
                (query.parents === undefined
                  ? true
                  : card.parentId !== null && query.parents.includes(card.parentId)) &&
        ...
        if (card.parentId !== null) {
          updateQueryData<GetCardData>(queryClient, getCardKey({ cardId: card.parentId }), (getCardData) => ({
            ...getCardData,
            childrenOrder: [card.id, ...getCardData.childrenOrder],
          }))
        }
      },
    }
...
export function useDeleteCard() {
  const queryClient = useQueryClient()
  return useMutation(
    async (data: DeleteCardBody) => {
      return callDeleteCard(data)
    },
    {
      onSuccess: (_void, variables) => {
        // Erase the GetCard query for this card
        queryClient.removeQueries(getCardKey({ cardId: variables.cardId }), {
          exact: true,
        })
        ...
        updateQueriesData<GetCardData>(
          ...
          (getCardData) => ({
            ...getCardData,
            childrenOrder: filterSync(getCardData.childrenOrder, (id) => id !== variables.cardId),
          })
        )
...
export function useMoveCard() {
  const queryClient = useQueryClient()
  return useMutation(
    async (data: MoveCardBody) => {
      return callMoveCard(data)
    },
    {
      onSuccess: (_void, variables) => {
...
export function useReorderCards() {
  const queryClient = useQueryClient()
  return useMutation(
    async (data: ReorderCardsBody) => {
      return callReorderCards(data)
    },
    {
      onSuccess: ({ childrenOrder }, variables) => {
        // Update the GetCard query
        updateQueryData<GetCardData>(queryClient, getCardKey({ cardId: variables.parentId }), (getCardData) => ({
          ...getCardData,
          childrenOrder,
        }))
