...
export async function canSeeCard<T extends { id: Card['id'], ownerId: Card['ownerId'] }>
  ...
  return cardSettings(fullCard).visibility === 'public'
    && (fullCard.parent === null || await canSeeCard(userId, fullCard.parent))
}
export function canEditCard<T extends { id: Card['id'], ownerId: Card['ownerId'] }>(userId: User['id'] | null, card: T) {
...
export async function canSeeComment<T extends { id: Comment['id'], ownerId: Comment['ownerId'] }>
  ...
  return commentSettings(fullComment).visibility === 'public'
...
export async function canSeeReply<T extends { id: Reply['id'], authorId: Reply['authorId'] }>
  ...
  return replySettings(fullReply).visibility === 'public'
    && await canSeeComment(userId, fullReply.comment)
}
export function canEditReply<T extends { id: Reply['id'], authorId: Reply['authorId'] }>(userId: User['id'] | null, reply: T) {
  if (!userId) return false // logged-out users cannot edit anything
  return reply.authorId === userId
}
export function canDeleteReply<T extends { id: Reply['id'], authorId: Reply['authorId'], comment: Pick<Comment, 'ownerId'> }>
  (userId: User['id'] | null, reply: T) {
