...
export type InboxItem = { tag: 'reply'; id: SubscriptionUpdate['id']; reply: Reply_ }

async function getUnreadReplies(userId: User['id']): Promise<InboxItem[]> {
  const items: InboxItem[] = await prisma.subscriptionUpdate
    .findMany({
      where: {
        subscriberId: userId,
...
async function getUnreadRepliesCount(userId: User['id']): Promise<number> {
  const items = await prisma.subscriptionUpdate
    .findMany({
      where: {
        subscriberId: userId,
        updateKind: 'suk_reply',
